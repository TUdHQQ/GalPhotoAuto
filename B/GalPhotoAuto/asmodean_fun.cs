...
namespace GalPhotoAuto
{
	...
	{
		// Token: 0x0600027E RID: 638 RVA: 0x000276B8 File Offset: 0x000258B8
		...
		// Token: 0x0600027F RID: 639 RVA: 0x00027744 File Offset: 0x00025944
		...
		// Token: 0x06000280 RID: 640 RVA: 0x000278C4 File Offset: 0x00025AC4
		...
		// Token: 0x06000281 RID: 641 RVA: 0x00027AA0 File Offset: 0x00025CA0
		...
		// Token: 0x06000282 RID: 642 RVA: 0x00027CE0 File Offset: 0x00025EE0
				...
				foreach (string text in array)
				{
					...
					if (0 < length)
					{
						...
						for (int j = num; j <= num2; j++)
						{
							...
							int num4 = (int)asmodean_fun.checkByte((int)buff[j] + num3);
							if (0 == j)
							{
								num3 = (int)Convert.ToByte(text[(j + 1 + 16) % length]);
								int num5 = (int)asmodean_fun.checkByte((int)buff[j + 1] + num3);
								num3 = (int)Convert.ToByte(text[(j + 2 + 16) % length]);
								int value = (int)asmodean_fun.checkByte((int)buff[j + 2] + num3);
								num3 = (int)Convert.ToByte(text[(int)((len - 1L + 16L) % unchecked((long)length))]);
								int num6 = (int)asmodean_fun.checkByte((int)buff[(int)(len - 1L)] + num3);
		...
		// Token: 0x06000283 RID: 643 RVA: 0x00027E7C File Offset: 0x0002607C
			...
			checked
			{
				foreach (string text in array)
				{
					...
					if (0 < length)
					{
						...
						{
							int num3 = (int)Convert.ToByte(text[(j + 16) % length]);
							buff[j] = asmodean_fun.checkByte((int)buff[j] + num3);
						}
					}
				}
				return buff[0] == 0;
		...
		// Token: 0x06000284 RID: 644 RVA: 0x00027F08 File Offset: 0x00026108
		private static byte checkByte(int ib)
		{
			checked
			{
				if (ib >= 256)
		...
		// Token: 0x06000285 RID: 645 RVA: 0x00027F28 File Offset: 0x00026128
		...
		// Token: 0x06000286 RID: 646 RVA: 0x0002812C File Offset: 0x0002632C
				...
				using (FileStream fileStream = new FileStream(lsf, FileMode.Open, FileAccess.Read, FileShare.Read))
				{
					...
					if (0 == string.Compare(strA, "LSF", StringComparison.Ordinal))
					{
						...
						for (int i = num; i <= num2; i++)
						{
							...
							{
								PublicModule.JpEncode.GetString(binaryReader.ReadBytes(64)).TrimEnd(new char[]
								{
									'\0'
								}),
								Conversions.ToString(binaryReader.ReadInt32()),
